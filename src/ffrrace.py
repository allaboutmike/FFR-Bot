...
class Race:
    ...
    def addRunner(self, runnerid, runner):
        if not self.islocked:
            self.runners[runnerid] = dict(
                [("name", runner), ("stime", None), ("etime", None), ("ready", False)]
            )
        else:
            raise RaceLocked
    ...
    def done(self, runnerid):
        ...
        rval = timedelta(
            microseconds=round(etime - self.runners[runnerid]["stime"], -3) // 1000
        )
        ...
        for runner in self.runners.values():
            ...
            if self.started:
                if runner["etime"] is maxsize:
                    ...
                    time = timedelta(
                        microseconds=round(runner["etime"] - runner["stime"], -3)
                        // 1000
                    )
                    rval += "done: " + str(time)
                else:
    ...
    def getTime(self):
        for i in self.runners.values():
            return timedelta(
                microseconds=round(time.perf_counter_ns() - i["stime"], -3) // 1000
            )
