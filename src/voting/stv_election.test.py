...
class TestStvElection(unittest.TestCase):
    ...
    def test_submit_vote(self):
        ...
        self.assertEqual(str(election.voters), r"{'321': 321 test name {'1': '123'}}")
    ...
    def test_get_winners(self):
        ...
        for i in range(500):
            choice = i % 5
            x = str(i)
            election.submit_vote(
                x + "voterid", x + "votername", [str(1) + ",," + str(choice)]
            )
        self.assertTrue(
            election.get_winners()["winners"] == set(["0", "1", "2", "3", "4"])
        )
